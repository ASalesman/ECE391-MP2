...
void octree_calculate_palette(uint8_t palette[192][3], octree_node_t *octree)
{
	...
	for (i = 0; i < 128; ++i) {
		...
		if (node->child_count) {
			...
			palette[i][1] = (node->green_total / node->child_count) >> 2;
			palette[i][2] = (node->blue_total  / node->child_count) >> 2;
		}
	...
	for (i = 0; i < 64; ++i) {
		...
		if (node->child_count) {
			...
			palette[i + 128][1] = (node->green_total / node->child_count) >> 2;
			palette[i + 128][2] = (node->blue_total  / node->child_count) >> 2;
		}
	}
}
